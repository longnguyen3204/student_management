<?php
$title = 'Chỉnh sửa sinh viên'; // Title of the page
...
// Retrieve the account details based on the student ID passed through the URL
$account = getAccountbyID($pdo, $_GET['id']);

// Check if the form is submitted to edit the student
if(isset($_POST['btn_edit_student'])){
    ...
    if (empty(trim($_POST['new_username'])) || empty(trim($_POST['new_email'])) || empty(trim($_POST['new_phone']))) {
        ...
        header('location: edit-student-code.php?id=' . $account['id']); // Redirect back to the student management page
        ...
        header('location: edit-student-code.php?id=' . $account['id']);
        exit();
    } else {
        
        // Check if the new email already exists in the system for a different student
        ...
        if ($check_email > 0) {
            ...
            // Proceed to update the student account in the database
            ...
            if($run) {
                ...
                header('location: student-manage-code.php'); // Redirect to the student management page
                exit(); // Exit the script
            } else {
                $_SESSION['status'] = 'Bạn cần phải chỉnh sửa!'; // Error message
                header('location: edit-student-code.php?id=' . $account['id']); // Redirect back
                exit();
            }
        }
    }
...
// Include the student edit form template
include BASE_PATH . '/templates/admin-temp/student-edit.html.php';

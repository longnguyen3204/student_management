<?php
$title = 'Edit Notification'; // Title of the page
...
if(isset($_POST['btn_edit_student'])){
    ...
    if (empty(trim($_POST['new_username'])) || empty(trim($_POST['new_email'])) || empty(trim($_POST['new_phone']))) {
        ...
        header('location: edit-student-code.php?id=' . $account['id']); // Redirect back to the user management page
        ...
        if ($check_email > 0) {
            ...
            if($run) {
                ...
                header('location: student-manage-code.php'); // Redirect to the user management page
                exit(); // Exit the script
            } else {

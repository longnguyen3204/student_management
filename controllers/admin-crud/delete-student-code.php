<?php
$title = 'Xóa sinh viên'; // Title of the page
...
// Check if the form is submitted to delete a student
if(isset($_POST['btn_delete_student'])){
    
    // Call the deleteAccount function to remove the student from the database
    $run = deleteAccount($pdo, $_POST['id']);
    
    if($run) {
        $_SESSION['status'] = 'Xóa tài khoản thành công!'; // Success message
        header('location: student-manage-code.php'); // Redirect back to the student management page
